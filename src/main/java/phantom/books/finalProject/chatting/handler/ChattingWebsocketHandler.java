package phantom.books.finalProject.chatting.handler;
...
public class ChattingWebsocketHandler extends TextWebSocketHandler{
  ...
  // synchronizedSet : 동기화된 Set (충돌 방지, 속도 조금 느림)
   ...
   // -> 연결된 클라이언트를 목록화
	...
	// 클라이언트로부터
	// SockJS.send() 구문을 이용해 텍스트 메세지가 전달된 경우
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		msg.setSenderNo(senderNo); // Mesaage 객체에 세팅
		
		// -------------DB INSERT-----------------------
		//1) ChattingService 의존성 주입받기(필드)
		
		// 2) insert하는 service호출
		...
		for(WebSocketSession wss : sessions) {
			...
			// 채팅방에 입장한 사람들(보낸/받는 사람)에게만
			// 메시지(msg)를 전달
			HttpSession clientSession
				= (HttpSession)wss.getAttributes().get("session");
			
			// 웹소켓 접속 회원 목록에서 꺼낸 회원번호
			int clientNo = ((Member)clientSession.getAttribute("loginMember"))
											.getMemberNo();
